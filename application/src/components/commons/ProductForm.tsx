...
import { UnidadeMedida, Produto, saveProdutoToLocalStorage } from '@/utils/localStorageUtil';
...
const ProductForm: React.FC<ProductFormProps> = ({ onSave, onCancel, initialData }) => {
    const router = useRouter();

    const [nomeProduto, setNomeProduto] = useState<string>(initialData?.nomeProduto || '');
    const [unidadeMedida, setUnidadeMedida] = useState<UnidadeMedida>(initialData?.unidadeMedida || UnidadeMedida.Litro);
    ...
    const [produtoPerecivel, setProdutoPerecivel] = useState<boolean>(initialData?.produtoPerecivel || false);
    const [dataValidade, setDataValidade] = useState<string>(initialData?.dataValidade || '');
    const [dataFabricacao, setDataFabricacao] = useState<string>(initialData?.dataFabricacao || '');

    useEffect(() => {
        // Atualizar os estados ao alterar o produto inicial
        if (initialData) {
